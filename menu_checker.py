...
■ 実装済みルール（マスタ照合ベース）
  ...
  No.15 健康食材 週1回以上
  No.17 1食で赤・黄・緑を使用（野菜マスタの色列・昼夜別。卵焼き(黄)は毎日必ず入っている
        前提のため、黄は常に満たすものとして扱う＝実質赤・緑のみ判定）
...
def check_rule17(data):
    ...
    卵焼き（黄）は、メニュー名や食材データ上には明記されない日でも毎日必ず1食に入っている
    前提のため、黄は常に満たしているものとみなす（ユーザー確認済み）。
    そのため実質的な判定は赤・緑の2色のみとなる。
    ...
    for d in dr:
        ...
        for slot, shoku in [('昼', data.shoku.get(month)), ('夜', data.shoku_night.get(month))]:
            ...
            # 卵焼き（黄）は食材データに現れない日でも毎日入っている前提のため、常に満たす扱いにする
            colors.add('黄')
            ...
            if missing:
                ...
                viol.append({
                    ...
                    '理由': f'{"".join(missing)}系の食材が0品（野菜マスタ照合。黄は卵焼き前提で常に満たす扱い）',
                    '修正提案': suggestion, '重要度': '中',
                })
    return pd.DataFrame(viol)
