...
■ 実装済みルール（マスタ照合ベース）
  ...
  No.15 健康食材 週1回以上
  No.17 1食で赤・緑を使用（野菜マスタの色列・昼夜別。黄(卵焼き)は毎日必ず入っている前提のため
        問答無用で常に満たすものとして扱い、判定対象から完全に除外 → 実質赤・緑の2色のみ判定）
...
# No.17用：判定対象の色。黄（卵焼き）は、メニュー名や食材データ上に一切現れない日でも
# 毎日必ず1食に入っている前提のため、問答無用で「常に満たしている」ものとして扱い、
# 野菜マスタとの照合対象から完全に外す（ユーザー確認済み）。
# そのため、以後 No.17 で実際にチェックされるのは赤・緑の2色だけになる。
REQUIRED_VEG_COLORS_NO17 = ('赤', '緑')


def check_rule17(data):
    """No.17: 1食につき赤・緑の食材を必ず使用する
    （黄＝卵焼きは毎日必ず入っている前提のため、問答無用で常に満たすものとして扱い、
    野菜マスタ照合や不足判定の対象から完全に除外している。REQUIRED_VEG_COLORS_NO17参照）。
    ...
    for d in dr:
        ...
        for slot, shoku in [('昼', data.shoku.get(month)), ('夜', data.shoku_night.get(month))]:
            ...
            # 黄（卵焼き）は野菜マスタと照合せず、最初から不足判定の対象に含めない。
            # REQUIRED_VEG_COLORS_NO17 = ('赤', '緑') のみを見る。
            missing = [c for c in REQUIRED_VEG_COLORS_NO17 if c not in colors]
            if missing:
                ...
                viol.append({
                    ...
                    'ルール': '1食につき赤・緑の食材を必ず使用（黄は卵焼き前提でチェック対象外）',
                    '該当箇所': f'[{slot}]',
                    '理由': f'{"".join(missing)}系の食材が0品（野菜マスタ照合。黄は判定対象から除外済み）',
                    '修正提案': suggestion, '重要度': '中',
                })
    return pd.DataFrame(viol)
...
ALL_RULES = [
    ...
    ('No.17 1食で赤・緑を使用（黄は判定対象外）', check_rule17),
...
RULE_NAME_JP = {
    ...
    16: '固形は2種まで', 17: '赤・緑を使用（黄は対象外）', 18: '1食の重量下限',
