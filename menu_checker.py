...
■ 実装済みルール（マスタ照合ベース）
  ...
  No.17 1食で赤・黄・緑を使用（野菜マスタの色列・昼夜別。黄(卵焼き)は毎食入っている前提のため
        常にクリア済みとして扱う。赤は赤パプリカ/カニカマ/花がんもも野菜マスタ未登録でも加味）
...
RED_SUPPLEMENT_KW_NO17 = ['赤パプリカ', 'カニカマ', '花がんも']
...
def check_rule17(data):
    ...
    また、赤パプリカ・カニカマ・花がんもは野菜マスタ未登録でも「赤」を満たすものとみなす
    ...
    for d in dr:
        ...
        for slot, shoku in [('昼', data.shoku.get(month)), ('夜', data.shoku_night.get(month))]:
            ...
            for prod in sub['商品名'].astype(str):
                ...
                # 赤パプリカ・カニカマ・花がんもは野菜マスタ未登録でも「赤」を満たす扱いにする
            ...
            if missing:
                ...
                viol.append({
                    ...
                    '該当箇所': f'[{slot}]',
                    '理由': f'{"".join(missing)}系の食材が0品（野菜マスタ照合。赤は赤パプリカ/カニカマ/花がんもも加味）',
                    '修正提案': suggestion, '重要度': '中',
                })
    return pd.DataFrame(viol)
