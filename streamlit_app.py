...
FETCH_TIMEOUT = 20   # 1件あたりの取得タイムアウト（秒）。長すぎると起動が固まるため短めにする
...
with st.expander('入力ファイルと設定', expanded=('result' not in st.session_state)):
    ...
    # ---- マスタ・過去メニューの接続状態 ----
    #   ここで取得しに行くと、ページを開くたびに9ファイル分のダウンロードが走り、
    #   画面が表示されるまで固まる（Streamlit Cloudでは起動失敗に見える）。
    #   そのため取得は「チェックを実行」時か、明示的に確認ボタンを押したときだけ行う。
    status = st.session_state.get('conn_status')
    if status:
        st.markdown(
            '<div class="statusline"><b>マスタ</b>　' + '　'.join(status['master_chips']) + '</div>'
            '<div class="statusline"><b>過去メニュー</b>　' + '　'.join(status['hist_chips'])
            + '　<span class="muted">※判定には使わず、代替え案の候補にのみ使用</span></div>',
            unsafe_allow_html=True)
    else:
        st.markdown(
            '<div class="statusline"><b>マスタ・過去メニュー</b>　'
            '<span class="muted">実行時にスプレッドシートから自動取得します'
            '（先に確認したい場合は右の「接続を確認」）</span></div>',
            unsafe_allow_html=True)

    o1, o2, o3, o4 = st.columns([1.1, 1.1, 1.6, 1.4])
    with o1:
        check_conn = st.button('接続を確認', use_container_width=True)
    with o2:
    ...
    # 手動アップロード欄は、接続確認で失敗したマスタがある場合だけ出す
    manual_paths = {}
    failed = (status or {}).get('master_failed', [])
    if failed:
        st.caption('取得できなかったマスタを手動で指定してください')
        m1, m2, m3 = st.columns(3)
        ups = {
            'veg': m1.file_uploader('野菜マスタ', type=['xlsx', 'csv'],
                                    disabled='veg' not in failed),
            'seasoning': m2.file_uploader('調味料', type=['csv'],
                                          disabled='seasoning' not in failed),
            'shoku': m3.file_uploader('食材データ', type=['xlsx'],
                                      disabled='shoku' not in failed),
        }
        for key, up in ups.items():
            if up is not None:
                manual_paths[key] = save_upload(up, 'master')


def load_sources():
    """マスタと過去メニューを取得し、状態を session_state に残して返す。
    ページ表示時ではなく、実行時・確認時にだけ呼ぶ。"""
    master_paths, master_failed, chips = {}, [], []
    for key in MASTER_META:
        path, msg, ok = resolve_master(key)
        master_paths[key] = path
        if not ok:
            master_failed.append(key)
        chips.append(f'<span class="{"ms-ok" if ok else "ms-ng"}">'
                     f'{"●" if ok else "▲"} {msg}</span>')
    hist_paths, hist_msgs, hist_labels = resolve_history()
    hchips = [f'<span class="{"ms-ok" if ok else "ms-ng"}">'
              f'{"●" if ok else "▲"} {msg}</span>' for ok, msg in hist_msgs]
    st.session_state['conn_status'] = {
        'master_chips': chips, 'hist_chips': hchips, 'master_failed': master_failed,
    }
    return master_paths, hist_paths, hist_labels


if check_conn:
    with st.spinner('スプレッドシートに接続しています…'):
        load_sources()
    st.rerun()
...
if run:
    ...
    with st.spinner('マスタ・過去メニューを取得しています…'):
        master_paths, history_paths, history_labels = load_sources()
        master_paths.update(manual_paths)   # 手動指定があれば優先
